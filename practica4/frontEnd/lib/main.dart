...
void main() {
  ...
  for (int i = 0; i < productos.length; i++) {
    ...
    print("Producto:" +
        ...
        descuento +
        ". Promo Aplicada: " +
        productos[i].getpromoAplicada().toString());
  }

  GestorFiltros gestorFiltros = inicializarGestorFiltros(productos);
  Cesta cesta = Cesta(gestorFiltros, usuario);
  ...
  for (int i = 0; i < productosFiltrados.length; i++) {
    ...
    print("Producto:" +
        ...
        descuento +
        ". Promo Aplicada: " +
        productosFiltrados[i].getpromoAplicada().toString());
  }
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<String> usuarios = ["Alejandro", "Anabel", "Elena", "Jose Luis"];
  ...
  late Cesta cesta = Cesta(gestorFiltros, user!);
  ...
  void dropdownButtonUsuario(String? newValue) {
    setState(() {
      user = newValue;
    });
  }
  ...
  void aplicarPromocion(Producto producto) {
    if (producto.id != null && producto.getPromoAplicada() == false) {
      cesta.aplicarPromocion(producto).then((_) {
        setState(() {});
      });
    }
  }
  ...
  void elevatedButtonConfirmarCallback() {
    cesta.subirCesta().then((_) {
      cesta.cargarProductos().then((_) {
        setState(() {});
      });
    });
        ...
        actions: <Widget>[
          DropdownButton<String>(
            ...
            onChanged: (String? newValue) {
              if (newValue != null && newValue != user) {
                user = newValue;
                cesta = Cesta(gestorFiltros, user!);
                cesta.cargarProductos().then((_) {
                  setState(() {});
                });
              }
          ...
          children: <Widget>[
            ...
            Row(
                ...
                children: <Widget>[
                  ...
                  ElevatedButton(
                    onPressed: () {
                      elevatedButtonConfirmarCallback();
                    },
                    child: const Text("Confirmar cesta"),
                  ),
                ]),
            Expanded(
              child: ListView.builder(
                itemCount: cesta.productos.length,
                itemBuilder: (context, index) {
                  Producto producto = cesta.productos[index];
                  return ListTile(
                    ...
                    trailing: Row(
                      ...
                      children: <Widget>[
                        ...
                        Text("Precio: " + producto.getPrecio().toString() + "€",
                            style: TextStyle(
                              fontSize: 16,
                              fontWeight: FontWeight.bold,
                            )),
